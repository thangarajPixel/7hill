...
const ProductFilters = () => {
    ...
    return (
        <>
          <h5 className="heading5"><Link className="product-filter-btn" onClick={ToggleClass}><RiFilter2Fill /> Filters</Link></h5>
          <Form className={isActive ? "product-filters" : "active product-filters"}>
            <Link className="close-btn" onClick={ToggleClass}><IoMdClose /></Link>
            {['checkbox'].map((type) => (
                <div key={`default-${type}`}>                    
                    ...
                    <Form.Check type={type} label={`Hydraulic Storage`} id={`hydraulicstorage`} />
                    <div className="divider"></div>
                    ...
                    <Link to="/" className="clear-btn">Clear All</Link>
                </div>
            ))}
        </Form>
